...
const companies = [
  // TODO: для удобства кода стоит отрефакторить его таким образом, чтобы это был не массив объектов, а объект с ключами-idшниками, значения которых уже будут этими самыми объектами - предположительно, так будет сильно проще обращаться к объекту, чем имеющееся. 
  {
    ...
    infoRus: 'Машиностроительная компания, выпускающая транспортные средства для земли, воды и воздуха, а также машинное оборудование для заводов.<br><br>В последнее время ведёт активную разработку беспилотных видов транспорта для повседневного пользования.',
  ...
  },
  {
    ...
    infoRus: 'Производство и продажа медикаментов, мировой лидер в разработке лекарств от любых недугов.<br><br>Компания стала особенно известна после представления миру высокоэффективного средства против ряда онкологических заболеваний.',
  ...
  },
  {
    ...
    infoRus: 'Новостная компания, непредвзято и честно освещающая все мировые события.<br><br>Имеет множество локальных филиалов в каждой стране и огромный штат в три тысячи сотрудников.',
    industry: ['social'],
  },
  {
    id: 3,
    ...
    infoRus: 'Лидер в области онлайн-образования.<br><br>На платформе доступно более 3000 курсов из самых различных областей, сертификаты об окончании курсов компании высоко котируются по всему миру.',
    industry: ['education'],
  },
  {
    id: 4,
    ...
    infoRus: 'Производство химического сырья широкого спектра - от реактивов высокой чистоты для химических лабораторий до простых соединений для пищевой промышленности и бытовых целей.',
  ...
  },
  {
    ...
    infoRus: 'Производитель кормов для животных (и не только), известный по всему миру.<br><br>Идеально оптимизированное производство качественного продукта по доступным ценам.',
  ...
  },
  {
    ...
    infoRus: 'Эталоны изысканных ароматов, известные всему миру - дело рук мастеров этой компании.<br><br>Законодатели моды в мире парфюмерии, многократный победитель FiFi Awards.',
    industry: ['chemicals'],
  },
  {
    id: 7,
    ...
    infoRus: 'Скандально известная юридическая компания, занимающаяся решением юридических задач любой сложности «под ключ».<br><br>Средний процент успешно завершённых дел под кураторством специалистов компании - 94%.',
    industry: ['social'],
  },
  {
    id: 8,
    ...
    infoRus: 'Компания, продукцию которой каждый день употребляют в пищу практически все.<br><br>Занимается массовым производством пищевой продукции, а также сельскохозяйственного сырья и удобрений. ',
    industry: ['food', 'agriculture'],
  },
  {
    id: 9,
    ...
    infoRus: 'Разработчик программного обеспечения и передовых технологических решений в области схемотехники.<br><br>Подавляющее большинство электронных устройств в мире работает и существует в привычном нам виде благодаря этой компании. ',
...
companies.forEach(({id, name, ticker, initPrice, info, infoRus, industry}) => {
...
const controlPanelSharesSetBar = document.getElementById('control-panel__shares-set-bar');
const controlPanelBalance = document.getElementById('control-panel__balance-section__balance');
const controlPanelCurrentPortfolioPrice = document.getElementById('control-panel__balance-section__current-portfolio-price');
const controlPanelOutcome = document.getElementById('control-panel__balance-section__outcome');
const controlPanelIncome = document.getElementById('control-panel__balance-section__income');
...
class InvestmentPortfolio {
  ...
  buyItem(id, companies) {
    ...
    if (!this.items[item.name]) this.items[item.name] = {'sharesCount': 0, 'id': id};
    const itemObj = this.items[item.name];
    itemObj.sharesCount += 1;
    this.balance -= Number(document.getElementById(`price-${id}`).textContent.replace('$', ''));

    document.getElementById(`control-panel__shares-set-bar-${id}-span`).textContent = itemObj.sharesCount;
    const timestamp = Date.now();
    controlPanelLog.insertAdjacentHTML("afterbegin", `<span id="temp-log-${timestamp}" style="color: darkred;">Bought "${item.name}" bond</span>`);
    controlPanelOutcome.insertAdjacentHTML("afterbegin", `<span id="temp-outcome-${timestamp}" style="color: red;">-$${Number(document.getElementById(`price-${id}`).textContent.replace('$', ''))}</span>`);
  ...
  sellItem(id, companies) {
    ...
    if (!this.items[item.name] || this.items[item.name].sharesCount < 1) {
      const timestamp = Date.now();
      controlPanelLog.insertAdjacentHTML("afterbegin", `<span id="temp-nothing-to-sell-${timestamp}" style="color: darkblue;">Nothing to sell</span>`);
      elementSmoothHide(document.getElementById(`temp-nothing-to-sell-${timestamp}`));
      return;
    }
    const itemObj = this.items[item.name];
    itemObj.sharesCount -= 1;
    ...
    document.getElementById(`control-panel__shares-set-bar-${id}-span`).textContent = itemObj.sharesCount;
    const timestamp = Date.now();
    controlPanelLog.insertAdjacentHTML("afterbegin", `<span id="temp-log-${timestamp}" style="color: darkgreen;">Sold "${item.name}" bond</span>`);
    controlPanelIncome.insertAdjacentHTML("afterbegin", `<span id="temp-income-${timestamp}" style="color: green;">+$${Number(document.getElementById(`price-${id}`).textContent.replace('$', ''))}</span>`);
...
[...document.getElementsByClassName('product-card__button-buy')].forEach(
  (btn) => {
    btn.addEventListener('click', e => {
      if (Number(document.getElementById(`price-${e.target.id.replace(/\D*/, '')}`).textContent.replace('$', '')) < Number(portfolio.balance)) {
        ...
        const timestamp = Date.now();
        controlPanelLog.insertAdjacentHTML("afterbegin", `<span id="temp-not-enough-money-${timestamp}" style="color: darkred;">Not enough money</span>`);
        elementSmoothHide(document.getElementById(`temp-not-enough-money-${timestamp}`));
      }
    })
  });
...
const pricesChange = () => {
  for (let i of [...document.querySelectorAll('.product-card__price')]) {
    ...
    const initPriceForCurrentProduct = companies.find(item => item.id === Number(i.id.replace(/\D*/, ''))).initPrice;
    let newPrice = currentPrice * ((90 + (Math.random() * 20)) / 100);
    if (newPrice > initPriceForCurrentProduct) newPrice -= ((newPrice - initPriceForCurrentProduct)**2)/7;
    if (newPrice < initPriceForCurrentProduct) newPrice += ((initPriceForCurrentProduct - newPrice)**2)/7;
...
const eventRandomPickFromEventsArray = () => {
    ...
    const currentRandomEvent = eventsClone[Math.floor(Math.random() * eventsClone.length)];
    let {text: currentRandomEventText, type: currentRandomEventType, subtype: currentRandomEventSubtype} = currentRandomEvent;
    ...
    const eventRandomPickFromEventsArrayPriceChangeHandler = () => {
      const tempId = companies.find(company => company.name == currentRandomCompanyName).id;
    ...
    // блок, если компания единственная в событии:
      ...
      const currentRandomCompanyName = companies[Math.floor(Math.random() * companies.length)].name;
      if (currentRandomEventText.includes('_company_')) {
        currentRandomEventText = currentRandomEventText.replace('_company_', currentRandomCompanyName);
      };
...
controlPanelCurrentPortfolioPrice.textContent = '$' + portfolio.marketPrice.toFixed(2);
